...
class WhatsappProxyhubHandler implements MessageHandlerInterface
{
    ...
    public function handleSendImage(Conversation $conversation, array $data): ?Message
    {
        ...
        try {
            ...
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $connection->credentials['token'],
            ])->post($this->base() . '/v1/message/send-image?instanceId=' . $connection->credentials['instance_id'], $payload);
    ...
    public function handleSendAudio(Conversation $conversation, array $data): ?Message
    {
        ...
        try {
            ...
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $connection->credentials['token'],
            ])->post($this->base() . '/v1/message/send-audio?instanceId=' . $connection->credentials['instance_id'], $payload);
    ...
    public function handleSendVideo(Conversation $conversation, array $data): ?Message
    {
        ...
        try {
            ...
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $connection->credentials['token'],
            ])->post($this->base() . '/v1/message/send-video?instanceId=' . $connection->credentials['instance_id'], $payload);
    ...
    public function handleSendDocument(Conversation $conversation, array $data): ?Message
    {
        ...
        try {
            ...
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $connection->credentials['token'],
            ])->post($this->base() . '/v1/message/send-document?instanceId=' . $connection->credentials['instance_id'], $payload);
    ...
    public function handleEditMessage(Message $message, array $data): ?Message
    {
        ...
        try {
            ...
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $connection->credentials['token'],
            ])->post($this->base() . '/v1/message/edit-message?instanceId=' . $connection->credentials['instance_id'], [
                'phone' => $conversation->external_id,
    ...
    public function handleDeleteMessage(Message $message): bool
    {
        ...
        try {
            ...
            $url = $this->base() . '/v1/message/delete-message?' . http_build_query([

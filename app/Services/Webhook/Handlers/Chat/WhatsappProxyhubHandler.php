...
class WhatsappProxyhubHandler implements ChatHandlerInterface
{
    ...
    {
        // Diagnostic: ProxyHub's inbound webhook payload is not documented in the
        // OpenAPI spec. Log its shape so we can confirm it matches the W-API format.
        Log::info('WhatsappProxyhubHandler: inbound webhook', [
            'connection_id' => $connection->id,
            'event' => $payload['event'] ?? null,
            'top_level_keys' => array_keys($payload),
            'has_data_envelope' => isset($payload['data']),
        ]);

        // Some providers wrap the event in a { data: {...} } envelope. If the top
        // level has no `event` but `data` does, unwrap it.
        if (! isset($payload['event']) && isset($payload['data']) && is_array($payload['data']) && isset($payload['data']['event'])) {
        ...
        if(!$event) {
            Log::warning('WhatsappProxyhubHandler: Event type is missing in payload', [
                'top_level_keys' => array_keys($payload),
            ]);
            return;
        }
        ...
        if($associationType === 'HD_VIDEO_DUAL_UPLOAD') {
            Log::info('WhatsappProxyhubHandler: Skipping HD video dual-upload child payload', [
        ...
        switch ($event) {
            ...
            default:
                // Don't 500 on an unrecognised event — ProxyHub would just retry.
                Log::warning('WhatsappProxyhubHandler: Unsupported event, ignoring', ['event' => $event]);
                break;
        }
    }
    ...
    private function handleEdited(Connection $connection, array $payload)
    {
        ...
        if(!$messageId || !$messageBody) {
            Log::warning('WhatsappProxyhubHandler: Missing required data in payload', [
        ...
        if(!$message){
            Log::warning('WhatsappProxyhubHandler: Edited message not found in database', [
                'message_id' => $messageId,
            ]);
    ...
    private function handleDeleted(Connection $connection, array $payload)
    {
        ...
        if(!$messageId) {
            Log::warning('WhatsappProxyhubHandler: Missing message ID in payload for deleted message', [
                'message_id' => $messageId,
            ]);
        ...
        if(!$message){
            Log::warning('WhatsappProxyhubHandler: Deleted message not found in database', [
                'message_id' => $messageId,
            ]);
    ...
    private function handleReceived(Connection $connection, array $payload)
    {
        ...
        if(isset($payload['msgContent']['messageStubType']) && in_array($payload['msgContent']['messageStubType'], ['CIPHERTEXT'])) {
            Log::info('WhatsappProxyhubHandler: Ignoring delivery receipt for message with unsupported stub type', [
        ...
        if($conversationId === 'status'){
            Log::info('WhatsappProxyhubHandler: Ignoring message with conversation ID "status"');
            return;
        }
        ...
        $message = DB::transaction(function() use ($connection, $payload, $conversationId, $messageId, $messageType, $contactExternalId, $contactName, $contactUsername, &$isNewConversation, &$conversationForWelcome) {
            ...
            if($conversation->messages()->where('external_id', $messageId)->lockForUpdate()->exists()) {
                Log::info('WhatsappProxyhubHandler: The message already exists, ignoring duplicate', [
                    'message_id' => $messageId,
                ]);
                return;
            }
            ...
            if ($repliedMessageExternalId) {
                ...
                if ($repliedMessage) {
                    ...
                    Log::warning('WhatsappProxyhubHandler: Replied message not found in database', [
            ...
            if ($isNewConversation && $conversationForWelcome) {
                if ($connection->flow_id) {
                    try {
                        ...
                        Log::error('WhatsappProxyhubHandler: Failed to start flow', [
                ...
                try {
                    ...
                    Log::error('WhatsappProxyhubHandler: Failed to resume flow', [
    ...
    {
        if($payload['isGroup'] ?? false) {
            Log::info('WhatsappProxyhubHandler: Skipping group reaction');
            return;
        }
        ...
        if (!$reactionData || !$reactionData['message_id']) {
            Log::warning('WhatsappProxyhubHandler: Missing reaction data', [
        ...
        try {
            ...
            if (!$targetMessage) {
                Log::warning('WhatsappProxyhubHandler: Target message not found for reaction', [
            ...
            if (empty($emoji)) {
                ...
                Log::info('WhatsappProxyhubHandler: Reaction removed', [
                ...
                Log::info('WhatsappProxyhubHandler: Reaction saved', [
                    'message_id' => $targetMessage->id,
                    'emoji' => $emoji,
                    'sender_type' => $senderType->value,
                ]);
            ...
            Log::error('WhatsappProxyhubHandler: Failed to handle reaction', [
                'error' => $th->getMessage(),
    ...
    private function handleDelivery(Connection $connection, array $payload)
    {
        ...
        if(!$fromMe) {
            Log::info('WhatsappProxyhubHandler: Ignoring delivery receipt for non-outgoing or non-API message');
            return;
        }
        ...
        if($fromMe && $fromApi && Message::where('external_id', $messageId)->exists()) {
            Log::info('WhatsappProxyhubHandler: Ignoring delivery receipt for message sent via API that already exists in database', [
                'message_id' => $messageId,
            ]);
            return;
        }
        ...
        $message = DB::transaction(function() use ($connection, $payload, $conversationId, $messageId, $messageType, $recipientId, $recipientName) {
            ...
            if(!$conversation){
                Log::info('WhatsappProxyhubHandler: Creating new conversation for delivery receipt', [
                    'conversation_id' => $conversationId,
                    'connection_id' => $connection->id,
    ...
    private function handleStatus(Connection $connection, array $payload)
    {
        ...
        if (!$messageId){
            Log::warning('WhatsappProxyhubHandler: Missing message ID', [
                'message_id' => $messageId,
            ]);
            return;
        }
        ...
        // if(!$fromMe) {
        //     Log::info('WhatsappProxyhubHandler: Ignoring status update for non-outgoing message');
        //     return;
        // }

        if(!$column) {
            Log::info('WhatsappProxyhubHandler: Ignoring unsupported status update', ['status' => $payload['status'] ?? null]);
            return;
        }
        ...
        if(!$message){
            Log::warning('WhatsappProxyhubHandler: Message not found for status update', [
                'message_id' => $messageId,
            ]);
            return;
        }
        ...
        Log::info('WhatsappProxyhubHandler: Updating status for message and previous outgoing messages', [
            'message_id' => $messageId,
    ...
    private function handleMediaMessage(Message $message, array $payload, MessageType $messageType)
    {
        ...
        if (!$mediaKeyValue || !$directPath || !$mimeType) {
            Log::error('WhatsappProxyhubHandler: Missing media data', [
        ...
        try {
            ...
            ])->post($this->base() . "/v1/message/download-media?instanceId=" . $message->conversation->connection->credentials['instance_id'], [
            ...
            Log::info('WhatsappProxyhubHandler: Media downloaded successfully', [
                'message_id' => $message->id,
                'media_path' => $mediaPath,
            ]);

        } catch (\Exception $e) {
            Log::error('WhatsappProxyhubHandler: Failed to handle media message', [
                'message_id' => $message->id,
                'error' => $e->getMessage(),
            ]);
    ...
    private function savePhotoProfile(Contact $contact, Connection $connection, array $payload)
    {
        ...
        if (!$profilePicture) {
            Log::info('WhatsappProxyhubHandler: No profile photo found for contact', [
                'contact_id' => $contact->id,
            ]);
            return;
        }
        ...
        try {
            ...
            if ($response->failed()) {
                Log::error('WhatsappProxyhubHandler: Failed to download profile photo', [
                    'contact_id' => $contact->id,
                    'url' => $profilePicture,
                ]);
                return;
            ...
            Log::info('WhatsappProxyhubHandler: Profile photo saved successfully', [
                'contact_id' => $contact->id,
                'photo_profile' => $photoPath,
            ]);

        } catch (\Exception $e) {
            Log::error('WhatsappProxyhubHandler: Failed to save profile photo', [
                'contact_id' => $contact->id,
                'error' => $e->getMessage(),
            ]);
        }
